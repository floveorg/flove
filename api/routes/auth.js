...
router.get('/me', requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT u.id, u.username, u.email, u.display_name, u.avatar_url, u.bio, u.website_url, u.created_at,
...
router.put('/profile', requireAuth, async (req, res) => {
  try {
    const { displayName, avatarUrl, bio, websiteUrl } = req.body;
    const { rows } = await pool.query(
      `UPDATE users SET
         ...
         updated_at = NOW()
       WHERE id = $5
       RETURNING id, username, display_name, avatar_url, bio, website_url`,
      [displayName, avatarUrl, bio, websiteUrl, req.user.userId]
    );
    const u = rows[0];
    res.json({
      id: u.id,
